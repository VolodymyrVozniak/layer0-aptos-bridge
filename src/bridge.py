...
from .settings import CHAIN_LIST


def aptos_bridge(name, private_key, from_chain, wallet, amount, max_gas, max_value):   
    log_name = f'APTOS BRIDGE {from_chain} to APTOS'
    to_chain = 'APTOS'

    from_data = search_setting_data(chain=from_chain, list=CHAIN_LIST)
    if len(from_data) == 0:
